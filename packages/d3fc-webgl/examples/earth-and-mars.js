...
const worker = new Worker('earth-and-mars-worker.js');
worker.onmessage = e => {
    data = e.data;
...
const blueGradientDecorate = program => {
    program.fragmentShader().appendBody(`
        ...
        gl_FragColor = vec4(color + ((vec4(1.0, 1.0, 1.0, 1.0) - color) * colorModifier));
    `);
};
...
const multiColorAreaDecorate = program => {
    program.vertexShader().appendHeader(`varying lowp vec4 vColor;`)
        .appendBody(`float colourModifier = smoothstep(50.0, 400.0, aYValue);
